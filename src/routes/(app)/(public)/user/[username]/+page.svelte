...
<svelte:head>
	...
	<meta name="description" content="View {data.user.name || data.user.username}'s profile and contributions to Svelte Society" />
</svelte:head>
			...
			<div class="flex-1">
				...
				<div class="mt-4 flex flex-wrap gap-4 text-sm text-gray-600">
					...
					{#if data.user.twitter}
						<a href="https://twitter.com/{data.user.twitter}" target="_blank" rel="noopener noreferrer" class="hover:underline">
							@{data.user.twitter}
						</a>
					{/if}
					...
					<div class="flex items-center gap-1">
						<Calendar size={16} />
						<span>Joined {new Date(data.user.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span>
					</div>
				</div>
