<script lang="ts">
	...
	const urlCount = $derived(() => {
		const urls = $formData.urls?.trim().split('\n').filter(line => line.trim()) || []
		return urls.length
	})
</script>
		...
		<div class="relative z-10">
			<div class="flex items-center gap-3">
				<div class="flex h-14 w-14 items-center justify-center rounded-xl bg-white/90 shadow-lg backdrop-blur-sm">
					<Stack class="h-7 w-7 text-svelte-500" weight="duotone" />
				</div>
				<div>
					<h1 class="text-3xl font-bold text-white">Bulk Import</h1>
					<p class="mt-1 text-sm text-white/90">
						Import multiple items at once for maximum efficiency
					</p>
				</div>
			</div>
		</div>
			...
			<div class="flex-1">
				...
				<div class="mt-3 grid gap-2 sm:grid-cols-2">
					<div class="rounded-lg bg-white p-3 shadow-sm">
						...
						<code class="mt-1 block rounded bg-svelte-100 px-2 py-1 font-mono text-xs text-svelte-900">
				...
				<div class="mb-6 flex items-center justify-between">
					...
					{#if urlCount() > 0}
						<div class="rounded-full bg-svelte-100 px-3 py-1">
							<span class="text-sm font-semibold text-svelte-900">{urlCount()} URL{urlCount() !== 1 ? 's' : ''}</span>
						</div>
					{/if}
				</div>
...
owner/repo-name"
						description="Enter YouTube URLs, video IDs, GitHub URLs, or owner/repo/path format. Maximum 50 URLs."
						rows={12}
					/>

					<div class="mt-6 flex items-center justify-between rounded-lg border-t border-gray-200 pt-6">
						<div class="flex items-center gap-2 text-sm text-gray-600">
							<Info class="h-4 w-4" weight="duotone" />
							<span>Processing in batches of {$formData.batchSize || 5}</span>
						</div>
						<Button type="submit" primary disabled={$submitting || urlCount() === 0}>
							{#if $submitting}
				...
				<div class="mt-6 space-y-3">
					<p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Supported Formats</p>
				...
				<div class="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
					...
					<div class="grid gap-3 sm:grid-cols-4">
						<div class="rounded-lg bg-gray-50 p-4 text-center">
							<p class="text-2xl font-bold text-gray-900">{importResults.summary.total}</p>
							<p class="text-xs text-gray-600">Total</p>
						</div>
						<div class="rounded-lg bg-green-50 p-4 text-center">
							<p class="text-2xl font-bold text-green-700">{importResults.summary.successful}</p>
							<p class="text-xs text-green-600">Successful</p>
						</div>
						<div class="rounded-lg bg-red-50 p-4 text-center">
							<p class="text-2xl font-bold text-red-700">{importResults.summary.failed}</p>
							<p class="text-xs text-red-600">Failed</p>
						</div>
						<div class="rounded-lg bg-yellow-50 p-4 text-center">
							<p class="text-2xl font-bold text-yellow-700">{importResults.summary.skipped}</p>
					...
					{#if importResults.results && importResults.results.length > 0}
						<div class="mt-6 space-y-2">
							<p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Detailed Results</p>
							<div class="max-h-96 space-y-2 overflow-y-auto rounded-lg border border-gray-200 bg-gray-50 p-3">
								{#each importResults.results as result}
									<div class="flex items-start gap-3 rounded-lg bg-white p-3 shadow-sm">
										{#if result.success}
											<CheckCircle class="h-5 w-5 shrink-0 text-green-600" weight="fill" />
										{:else}
											<XCircle class="h-5 w-5 shrink-0 text-red-600" weight="fill" />
										{/if}
										<div class="flex-1 min-w-0">
											<p class="truncate text-sm font-medium text-gray-900">{result.url}</p>
											{#if result.error}
												<p class="mt-0.5 text-xs text-red-600">{result.error}</p>
											{/if}
											{#if result.type}
												<span class="mt-1 inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700 capitalize">
													{result.type}
												</span>
											{/if}
										</div>
			...
			{#if $message}
				<div class="mt-6 rounded-xl border border-svelte-200 bg-svelte-50 p-4">
					<p class="text-sm text-svelte-900">{$message.text}</p>
				</div>
			{/if}
		</div>
		...
		<div class="space-y-6">
			...
			<!-- Import Stats -->
			<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
				<h3 class="mb-4 text-sm font-bold uppercase tracking-wide text-gray-700">Processing Speed</h3>
				<div class="space-y-4">
					<div>
						...
						<div class="h-2 overflow-hidden rounded-full bg-gray-100">
							<div class="h-full w-3/4 rounded-full bg-gradient-to-r from-red-500 to-red-600"></div>
						</div>
					</div>
					<div>
						<div class="mb-2 flex justify-between text-xs">
						...
						<div class="h-2 overflow-hidden rounded-full bg-gray-100">
							<div class="h-full w-2/3 rounded-full bg-gradient-to-r from-gray-700 to-gray-900"></div>
						</div>
					</div>
				</div>
				<p class="mt-4 rounded-lg bg-svelte-50 p-3 text-xs text-gray-700">
					<strong class="text-svelte-900">Tip:</strong> Larger batches are processed in parallel for faster imports.
				</p>
			</div>

			<!-- Requirements -->
			<div class="rounded-xl border border-yellow-200 bg-yellow-50 p-5 shadow-sm">
				<h3 class="mb-3 text-sm font-bold uppercase tracking-wide text-yellow-900">API Requirements</h3>
