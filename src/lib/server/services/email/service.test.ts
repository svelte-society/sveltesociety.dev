...
describe('EmailService', () => {
  // Base test layer with all mocks
  const makeTestLayer = () =>
  ...
  describe('send', () => {
    ...
    test('uses default from address from config', async () => {
      ...
      expect(sent[0].from).toBe('test@example.com') // From EmailConfigTest
    })
    ...
    test('returns failure when SES fails', async () => {
      const layer = EmailServiceLive.pipe(
        ...
        Layer.provide(EmailQueueLive),
        Layer.provide(KeyValueStoreTest),
        Layer.provide(makeEmailConfigTest({ maxRetries: 0 })) // No retries for faster test
      )
  ...
  describe('sendBatch', () => {
    ...
    test('respects maxBatchSize limit', async () => {
      ...
      const layer = EmailServiceLive.pipe(
        ...
        Layer.provide(EmailQueueLive),
        Layer.provide(KeyValueStoreTest),
        Layer.provide(makeEmailConfigTest({ maxBatchSize: 2 }))
      )
    ...
    test('handles partial failures', async () => {
      ...
      const layer = EmailServiceLive.pipe(
        ...
        Layer.provide(EmailQueueLive),
        Layer.provide(KeyValueStoreTest),
        Layer.provide(makeEmailConfigTest({ maxRetries: 0 }))
      )
  ...
  describe('retry behavior', () => {
    test('retries on failure with exponential backoff', async () => {
      ...
      const layer = EmailServiceLive.pipe(
        ...
        Layer.provide(EmailQueueLive),
        Layer.provide(KeyValueStoreTest),
        Layer.provide(makeEmailConfigTest({ maxRetries: 3 }))
      )
