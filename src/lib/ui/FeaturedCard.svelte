<script lang="ts">
	...
	import { titleVariants, descriptionVariants } from './contentCard.variants'
...
<article
	...
	class="grid min-w-0 gap-2 rounded-lg bg-gradient-to-r from-svelte-50 to-svelte-100 px-4 py-4 sm:px-6 sm:py-5"
>
	<div class="mb-2 grid grid-cols-[1fr_auto] items-center justify-between gap-2 text-xs sm:gap-0">
		<div class="flex min-w-0 flex-wrap items-center gap-2">
			...
			{:else if content.author_username}
				<span class="flex min-w-0 max-w-full text-gray-500">
					<span class="shrink-0">{content.type === 'video' || content.type === 'library' ? 'submitted' : 'posted'} by&nbsp;</span>
					<a
	...
	{#if content.type === 'job'}
		<div class="flex gap-4">
			...
			{:else}
				<div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-white/50 sm:h-20 sm:w-20">
					<Buildings size={32} class="text-slate-400" />
				</div>
			{/if}
			<div class="min-w-0 flex-1">
				<h2 data-testid="content-title" class={titleVariants({ variant: 'list' })}>
					<a
	...
	{:else}
		...
		{#if content.description && content.type !== 'resource'}
			<div data-testid="content-description" class={descriptionVariants({ variant: 'list' })}>
				{content.description}
			</div>
		{/if}
		...
		<div class="mt-2 min-w-0">
			...
			{:else if content.type === 'library'}
				<Library {content} {priority} variant="list" />
			{:else if content.type === 'announcement'}
				<Announcement {content} variant="list" />
			{:else if content.type === 'resource'}
				<Resource {content} {priority} variant="list" />
			{/if}
		</div>
	{/if}

	<div class="mt-4 grid grid-cols-1 items-center justify-between gap-2 sm:grid-cols-[1fr_auto] sm:gap-0">
		{#if content.type === 'job'}
