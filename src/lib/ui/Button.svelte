<script lang="ts">
	...
	const baseStyles =
		'inline-flex items-center justify-center gap-1 rounded-md px-4 py-2 font-medium focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-700 cursor-pointer'
</script>
...
<svelte:element
	...
	class={[
		baseStyles,
		{
			'bg-svelte-900 focus:ring-svelte-900 text-white hover:brightness-150': primary,
			'bg-gray-200 text-gray-900 hover:bg-gray-300 focus:ring-gray-500': secondary,
			'border-svelte-300 bg-svelte-100 text-svelte-900 focus:ring-svelte-900 border text-sm font-bold':
				tertiary,
			'bg-green-400 text-black hover:bg-green-200 focus:ring-green-500': success,
			'w-full': fullWidth,
			'px-6 py-3 text-lg': large,
			'px-2 py-1 text-xs': thin,
			'px-3 py-2 text-sm': small,
			'rounded-md bg-red-600 text-white transition-colors duration-200 hover:bg-red-700': error
		}
	]}
>
	{#if icon_left}
