...
Restore browsing during crawler bursts while retaining per-visitor and global crawler limits. This is a high-risk request-admission change across Nginx and Bun. The original checkout was behind upstream main; this branch starts at upstream `2e790b88`. No production logs or deployment settings have been inspected. Existing Cloudflare header trust assumes the origin is reachable only through trusted ingress.
...
- The origin's actual deployed revision, request logs and Cloudflare settings are still unverified. These results establish the code defect and regression fix, not a confirmed diagnosis of every production symptom.
